...
| `validate_slugs.py` | Checks every configured company's slug still resolves |
...
| `.github/workflows/watcher.yml` | Schedule definition for GitHub Actions |
...
If you added `.github/workflows/maintenance.yml`, this runs automatically every Monday and commits the pruned config — no local action needed at all.
...
Repo page → **Actions** tab → **Job Watcher** workflow → **Run workflow**. Useful for testing after a config change without waiting for the next scheduled tick.

### Monitoring the automation
Repo page → **Actions** tab shows every past run, green/red status, and full logs (including any fetch failures per company, which print to the log without stopping the rest of the run).
...
3. `.github/workflows/watcher.yml` handles the rest — runs every 20 minutes automatically once pushed.
...
- **Cron timing isn't exact** — GitHub Actions scheduled workflows can be delayed a few minutes under platform load; treat the 20-minute interval as approximate, not guaranteed.